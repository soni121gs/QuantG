...
- `[ ]` **IA-1** GEX / dealer-positioning signal — `core/dealer_positioning.py` (PURE, no I/O): from per-strike OI + settle (bhavcopy live + historical) compute daily NET gamma exposure per underlying + OI walls (pin strikes) + `gex_state` (POSITIVE=range/mean-revert, NEGATIVE=trend/vol-expansion, FLIP). Standard convention: retail net-long-calls/short-puts, dealers opposite. Historically reconstructable from the store (rule #1). Unit tests + a scratch validation that GEX sign correlates with the RAE day-label on the 494-day store. NOTHING wired yet — pure signal + evidence.
- `[ ]` **IA-2** Wire GEX as a CONFIDENCE input to the RAE classifier — `regime_classifier`: POSITIVE GEX raises RANGE confidence, NEGATIVE GEX raises TREND confidence (the precision lever RAE-3c needs — a trend call with negative-GEX confirmation is far more reliable than price-breakout alone). Additive, env-gated OFF by default (byte-identical no-op until enabled). Re-validate via `run_regime_oos_validation` that the sharpened confidence improves the trend specialist's on-regime precision.
