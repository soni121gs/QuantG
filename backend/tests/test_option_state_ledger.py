...
def _isolated_ledger(tmp_path, pool_size):
    db_name = f"quantg_pytest_{uuid.uuid4().hex}"
    env = {"QUANTG_TEST_DB": "true", "DB_NAME": db_name}
    patcher = patch.dict("os.environ", env)
    patcher.start()
    ledger = OptionStateLedger(tmp_path / "runtime.sqlite3", pool_size=pool_size)
    return ledger, patcher


def _close_isolated_ledger(ledger, patcher):
    try:
        ledger.client.drop_database(ledger.db_name)
    finally:
        ledger.close()
        patcher.stop()
...
def test_duplicate_buy_is_dropped_atomically(tmp_path):
    ledger, patcher = _isolated_ledger(tmp_path, pool_size=4)
    try:
        ledger.initialize()
        ...
        _close_isolated_ledger(ledger, patcher)


def test_close_moves_to_cooldown_and_journals_trade(tmp_path):
    ledger, patcher = _isolated_ledger(tmp_path, pool_size=2)
    try:
        ledger.initialize()
