...
QuantG use: ERP requires expected edge >= 3x modeled friction. Enforced at BUILD time in `core/spread_builder.credit_cost_floor` (2026-07-21), not only in research validators â€” the live path previously opened trades the Diagnostician had already condemned.
...
Round-trip friction is ~Rs300/lot (slippage on 4 legs + brokerage + taxes) â€” the figure `dynamic_exit.TRAIL_MIN_ARM_RUPEES` already encoded. An earlier probe constant of Rs85 understated it 3.5x, which is how sub-floor geometry passed `static.cost_floor`.
...
Practical tell: look at the long wing's price. QG-O1's was Rs0.46 while consuming Rs32,000/lot of risk budget â€” a naked short with a lottery ticket attached. A nearly worthless wing means the structure is not meaningfully defined-risk.
