...
- `[ ]` **IA-2** *(REVISED after IA-1)* GEX as FEATURES (not a raw-sign gate) — before any classifier wiring, run the follow-up study: (a) INVERTED sign convention, (b) OI-wall PIN behaviour (does price gravitate to max-OI strike?), (c) GEX magnitude/`chg_in_oi` day-over-day. Wire only the component that beats base rate on `run_regime_oos_validation`; otherwise hold GEX as a GBM feature (IA-5). Env-gated OFF, additive, byte-identical no-op until proven. Same discipline: prove-before-wire.
