...
- `[ ]` **EM-2** Rolling-stats provider — adapter over `trade_attribution.attribution_rollup` → per-(strategy, regime, vol_bucket) W/μ_win/μ_loss over rolling last-N CLOSED trades; cached per strategy per IST day. Feeds EM-1 primitives. Read-only on `trade_attribution` — no conflict.
- `[ ]` **EM-3** market_context adapter (live + historical) — make `core/market_context.build_context` callable at signal time to supply regime + vol_state + IV−RV for L1 conditioning and L3 budget scaling, identically live (feed) and in the OOS backtester (store). Reuses RES-2; already pure — no conflict.
- `[ ]` **EM-4** Wire sizing into `signal_manager` — replace `_spread_lots = lots_for_risk(...)` (`:845`/`:890`) with `edge_size()` output (base × conviction × day_mult), **capped to `risk_manager` capital + `capital_model` available margin** (growth must not exceed funds), and **subsume the legacy `alloc_mult`** (`:1110`). Paper-first, founder-gated. CONFLICT ZONE — touch carefully.
- `[ ]` **EM-5** Reconcile `profit_lock.py` (L3 overlap) — it currently HARD-BLOCKS re-entry (`day_profit_locked=True`, the ONLY writer; `signal_manager` reads+blocks). Per the no-gates mandate, convert arm/trail into the EM day-governor's continuous size-down (protect gains by shrinking, not blocking) OR keep profit_lock only as a far-ceiling backstop; REMOVE the re-entry block. Update both writer + reader.
- `[ ]` **EM-6** Volatility-scaled exits — TP/SL in σ_intraday/ATR units from market_context → `spread_lifecycle.compute_exit_levels`, additive to per-strategy `credit_tp_frac`/`credit_sl_mult` (keep env defaults). Targets breathe with vol so noise doesn't stop winners.
- `[ ]` **EM-7** OOS validation — drive `edge_size` through the bhavcopy/IMD backtester: does dynamic sizing beat FLAT sizing on OOS expectancy AND compress the daily-loss distribution (max daily DD, loss/profit ratio, % green days)? Pure-fn reuse. GATE: must beat flat before EM-4 goes live.
- `[ ]` **EM-8** Self-tuning ratchet — EOD job nudges per-strategy params toward positive-expectancy buckets (EM-2 stats); observe-only (`HERMES_ADVICE_ENABLED` off) → founder-gated apply. Reuses HSI/`hermes_advisor`. Advisory — no live-path conflict.
- `[ ]` **EM-9** Telemetry — surface `edge_score`, current size multiplier, day-governor state per strategy on Analytics/Execution so the sizing is inspectable, not a black box.
