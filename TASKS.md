...
- `[x]` **EDR-08** *(answered 2026-07-04)* Per-year OOS split of the SENSEX condor in the backtester: **wing=6 = CANDIDATE_EDGE, +₹318/trade @ 88% WR, n=33, 2024 +₹316 / 2025 +₹320** (OOS year holds, both years positive & consistent). wing=4 also passes (+₹109, 82%). **First CANDIDATE_EDGE in the whole program.** (Small sample — firm up with EDR-02's larger cadence.)
- `[ ]` **EDR-02** *(NOW UNBLOCKED — the headline next task)* Turn the validated SENSEX iron condor into a real, deployable strategy. Sub-tasks: **(a)** dedicated condor `python_code` that enters ~every weekly expiry (n≈100, firmer verdict) + a wing-width/short-OTM sweep to pick the config; **(b) LIVE-engine 4-leg support** — `core/option_selector_v2`, `core/spread_builder`, `core/paper_broker`, `position_monitor`/`guardian`, execution router currently handle only single_leg/credit/debit spread; iron_condor needs 4-leg entry/pricing/exit/monitoring; **(c)** seed as a paused SENSEX strategy (`structure=iron_condor`, `exit_mode=expiry`, no stop, hold-to-expiry, ~2% OTM, wing 6) → forward-paper 3-6 wks → live pilot. Tier 3. **Big chunk — (b) is the real work.**
