...
- [~] **TASK-AUTH-0909** (T3, Codex) Change existing Upstox approval prompts to 07:00 and 08:30 IST on trading weekdays; retain fresh-token skip, consult broker holiday calendar, deduplicate each daily slot across restarts. Files: upstox_auth_request.py, server.py scheduler, routes/broker.py status, focused tests. Verify auth tests, compilation, production schedule/status and service health; deploy using preserved clean-release workflow.
