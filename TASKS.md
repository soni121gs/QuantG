...
- [ ] **P0-1** (T2) History snapshot before ANY delete: JSON export of `strategies` + `strategy_positions` + `trade_fills` (+ signals counts) to `data/archive/book_snapshot_2026-07/`. Acceptance: restorable dump exists outside Mongo.
- [ ] **P0-2** (T3, =INFRA-13) Registry migration: move all code templates (`DEFAULT_OPTION_STRATEGIES` server.py:~3544, `STANDARD_STRATEGY_CATALOG` :~6598, `UPGRADED_DEFAULT_STRATEGY_CODE_BY_NAME`, per-strategy geometry blocks like `CREDIT_SPREAD_THETA_RISK`) into `core/strategy_registry` rows. Acceptance: every kept strategy exists as a registry row with identical geometry; code templates deleted.
- [ ] **P0-3** (T3, =INFRA-14) Kill the frozensets: `PAPER_FORWARD_ACTIVE/ARCHIVED_STRATEGY_NAMES`, `DEAD_STRATEGY_NAMES`, `OPTION_ALPHA_REBUILD_NAMES`, `CREDIT_SPREAD_THETA_NAMES`, `EQUITY_CAPITAL_TIERS`, `STRATEGY_DISPLAY_NAME_RENAMES` (server.py ~1615–1712) + startup enforcement (~17287–17311) → `registry.active` boolean. Acceptance: wake/archive = flip a DB field; the "half-woken book" class impossible.
- [ ] **P0-4** (T3, =INFRA-15) Remove startup template re-sync/migrations (`_migrate_strategy_code_versions`, structure migrations at :~16914/16949). Acceptance: a DB geometry edit persists across restart.
- [ ] **P0-5** (T2) ⛔D-1 Execute the purge (census: Edge Report §1): DELETE 9 index buyers/scalpers (−₹16,008/153tr) · QG-O2+O5..O10 (0 trades ever) · 4 old theta spreads (−₹6,093/122tr) · 10 equity intraday (−₹26,157/152tr) · QG-O3 (redundant) · MANUAL_RECOVERY row · MCX/commodity templates from code. Update catalog tests (`test_default_strategy_catalog_upgrade.py`, exit-mode matrix). Acceptance: DB holds only the 9 keepers (paused) + registry rows; tests green.
- [ ] **P0-6** (T2, =INFRA-17) Cutover verification: one clean paper session on the registry-backed book; no template resurrection on restart.
- [ ] **P0-7** (T2) Cost-floor tripwire: Diagnostician probe `static.cost_floor` (modeled friction — brokerage+STT+txn+GST+slippage — vs geometry's expected edge; FINDING if edge < 3× friction) + same check as an ERL promotion gate. Acceptance: a width-1 ₹20-credit design gets flagged automatically.
- [ ] **P0-8** (T2) Contract-spec drift tripwire: probe `infra.contract_spec_drift` — configured lot/expiry vs Upstox instrument master, daily — plus a runtime lot-resolution helper so `market_domains` values are verified, not trusted. Acceptance: a future exchange lot revision surfaces as a finding within one day.
- [ ] **P0-9** (T1) Doc hygiene: after P0-5, record the executed purge in CLAUDE.md §20; prune stale §2 file-map rows; AGENTS.md §2 already refreshed 2026-07-19.
