...
- [~] **P1-5** (T2) Options-minute backfill via Upstox **Expired Instruments APIs** (official). D-2 approved 2026-07-19. App alignment landed: `/api/upstox/data-health`, Broker Keys, Market Hub, Hermes Research Lab, and Hermes tool `get_upstox_data_health` now expose Plus capability + real store coverage. Importer now supports `--sleep-sec` throttling after overnight runs hit Upstox 429s. Remaining: run bounded deep `options_1m` chunks slowly and confirm coverage grows beyond 204 days + live capture.
