...
- **Book state (2026-07-19): 0 live, 7 paused, 34 archived.** The full census found the lifetime book = **−₹68,011 over 494 closed trades** with NO strategy having both n≥30 and positive P&L. A founder-gated purge of ~32 dead rows (including all 10 equity intraday strategies) is queued as TASKS.md Phase 0. Keep-list (paused, pending re-judge): QG-O1, QG-O4, QG-O11, 3 RAE range sellers, 3 RAE trend delta-1.
...
- **Live-book infra hardened (2026-07-06):** spread pyramiding stopped (anti-re-entry guard in `signal_manager.py`); spread P&L marks fixed (WS-subscribe `legs[]`, was 429→₹0.00); **manual exit + EXIT ALL now route through `_close_strategy_positions` — NEVER close a position via a generic opposite order under `manual_recovery` (ledger nets by strategy_id → phantom SHORT).** (The 2026-07-06 "book locked to QG-O1 + QG-O5" note is obsolete — see the book-state bullet above.) See CLAUDE.md §1 session-fixes + §6.
- **▶ ACTIVE PROGRAM (2026-07-19) — Edge Rebuild Program (ERP).** The master plan after the full census + Edge Reports v1–v3 (the RES §15 / EM §16 / RAE §18 machinery is CONSUMED by it; their task queues are archived). Phases in TASKS.md (rewritten 2026-07-19): **0** purge + Strategy-Registry migration + cost-floor & contract-spec-drift probes → **1** data (stock-F&O ingest, earnings calendar, 2019–23 backfill, participant OI) → **2** analytics (IV surface, Deflated Sharpe in ERL, event-conditional judge) → **3** sleeves (earnings IV-crush flagship, PEAD, daily delta-1 momentum/overnight, slow premium core, OI overlay) → **4** Hermes research analyst. New laws: cost-floor (expected edge ≥ 3× modeled friction at design time), breadth (Grinold-Kahn), trials-count/DSR on verdicts, verify-live-facts (lots verified 2026-07: NIFTY 65 / BANKNIFTY 30 / SENSEX 20 — check the instrument master, never model memory). Full spec: CLAUDE.md §20.
- **▶ NEXT BUILD MAP (2026-09-04) — Phase 6 Profitable Machine Program.** `TASKS.md` Phase 6 tracks the founder's 1-9 upgrade map: Alpha Factory, purged/embargoed OOS, Execution Quality Ledger, ML setup ranker, execution optimizer, graduation board v2, breadth campaigns, Hermes weekly analyst, and SEBI/algo audit readiness. `db.execution_quality` is now an evidence source for fill realism, slippage, charges, missed fills, and live-readiness promotion blockers. Telemetry must remain best-effort/read-only: it must never block a fill, mutate strategy status, wake paused rows, enable live mode, or change broker routing.
...
| Execution quality / slippage evidence | `backend/core/execution_quality.py` + `/ops/execution-quality` |
...
### Provenance and signal-audit law
