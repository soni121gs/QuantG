...
- [x] **TASK-AUDIT-0909** (T3, Codex; `4ab7b1b`, `f5babf1`) Engineering/accounting remediation of audit findings 1-7 deployed 2026-09-09: affordable/aggregate spread risk and correlated-expiry guards; terminal margin release; intrinsic settlement correction; corrected Hermes taxonomy/epochs/narration; current-book validation and missing-OOS promotion gate; pending telemetry/Founder UI deployed. Additional root cause fixed: Ops stale-order cleanup could void funded positions without reversing cashflows. Five audited cancelled entry cashflows reversed with backup/journal/CAS. Verification: 107 focused tests, frontend build, isolated Mongo concurrency/telemetry test, production wallet=canonical fills plus open cashflows (Rs457,546.21), margin drift zero, repeated repair no-op, Ops guard HTTP409, public version/health and healthy containers. Live flags false. Authenticated UI render remains unverified (only login render checked). See [[Infrastructure and Strategy Audit 2026-09-09]].
- [ ] **TASK-AUDIT-0909-R** (T3, research continuation) Finding 7 remains a research gate: implement and verify production exit-policy parity for multi-day spreads, acquire sufficient matched data, rerun purged OOS and collect fresh forward-paper/execution-quality evidence. No strategy has been newly validated or promoted by the infrastructure repair. Current 13-row intraday replay is explicitly promotion-ineligible; broad EOD rerun was stopped because it cannot validate intraday exits. Files: intraday/multi-day replay engine, validator, parity fixtures and research report. Verify exact entry/exit/config parity, modeled costs, sample size, net expectancy, OOS/DSR and subsequent paper evidence before any strategy deployment.
...
- [x] **P6-3** (T3) Execution Quality Ledger. Persist expected mid/bid/ask proxy, actual fill, modeled vs realized slippage, fill delay, missed-fill reason, and leg-sequence evidence for paper/live orders. Implemented `db.execution_quality`, `/ops/execution-quality`, Hermes `get_execution_quality`, Founder Machine cost metric, and promotion blockers for missing fill-quality evidence. Commit: `0d95e3c`.
...
- [~] **P6-10** (T3) Aladdin-style whole-portfolio intelligence (Codex). Build a read-only canonical portfolio snapshot for stocks, options and spreads; then add deterministic scenario/risk analytics, Hermes explanations, and an approval-gated cockpit in separate verified slices. **Landed slices:** `core.portfolio_intelligence`, `/ops/portfolio-snapshot`, Hermes `get_portfolio_snapshot`, pure aggregation tests, `core.portfolio_scenarios`, `/ops/portfolio-scenario` with explicit missing-input handling, and the `/portfolio-intelligence` read-only cockpit. No order, wallet, strategy, broker, or live-mode mutation is permitted.
...
- [x] **TASK-AUTH-0909** (T3, Codex; `16749b6`, deployed and verified) Change existing Upstox approval prompts to 07:00 and 08:30 IST on trading weekdays; retain fresh-token skip, consult broker holiday calendar, deduplicate each daily slot across restarts. Files: upstox_auth_request.py, server.py scheduler, routes/broker.py status, focused tests. Verify auth tests, compilation, production schedule/status and service health; deploy using preserved clean-release workflow.
