...
- `[~]` **EDR-02** *(a) DONE (2026-07-04, `/tmp/condor_sweep.py`): weekly-cadence condor + 18-config OTM×wing sweep on SENSEX+NIFTY — **0 CANDIDATE_EDGE, best SENSEX = FRAGILE**. **(b)/(c) NOT STARTED and now ON HOLD** — do NOT build the 4-leg live engine or seed a strategy for a FRAGILE structure. The defined-risk condor gives back too much of the naked short-vol edge (wings + year-inconsistency).* Tier 3.
  ...
  - `[ ]` **EDR-11 *(the remaining gap before forward-paper)*** Wire the LIVE hold-to-expiry path so the put spread actually trades its validated behavior: (1) OTM short-strike selection in `core/option_selector_v2` (sells ATM today — honor `short_otm_pct`/delta); (2) MULTI-DAY hold to weekly expiry — exempt this strategy from the 15:25 intraday squareoff, tp/sl, and staleness force-close (the engine is intraday today; this is the real change). Reuses the 2-leg credit_spread execution otherwise. Then flip DRAFT→paused/live for forward-paper 3-6 wks → live pilot. Tier 3.
