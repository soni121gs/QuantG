...
| Regime label survives a late/missing opening feed | `market_regime_state` must persist `regime_fine_data_quality`, `regime_fine_bar_count`, `regime_fine_first_bar_ist`, `regime_fine_source`, and `regime_fine_router_allowed`. `INSUFFICIENT_BARS`/`LATE_OPEN` block fine-regime consumers; never reuse yesterday's label. Fixed 2026-09-10 in the regime scheduler and signal manager. |
