...
- `[ ]` **RAE-0** Lock the regime taxonomy — freeze the 6-label set + thresholds from `scratch/regime_directional_oos.py` (`TREND_UP/TREND_DOWN/RANGE/HIGH_VOL_CHOP/INSIDE_QUIET/EVENT`) as canonical, with base rates. Everything downstream keys off this. Pure/doc — zero risk. *Deliverable: versioned label definitions + per-label base rates.*
- `[ ]` **RAE-1** The Regime Classifier — pure `regime_of(context) → (label, confidence)`, no-lookahead, IDENTICAL live (feed) and historical (store) per the RES design rule. Reuse `market_context.py` (IV−RV, vol_state, regime, chain, events) + `historical_regimes.py`. Confidence is required (not just a label). ⛔ needs RAE-0. *Deliverable: every past day + every live minute gets a labeled regime + confidence.*
