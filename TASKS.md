...
- `[~]` **EDR-02** *(a) DONE (2026-07-04, `/tmp/condor_sweep.py`): weekly-cadence condor + 18-config OTM×wing sweep on SENSEX+NIFTY — **0 CANDIDATE_EDGE, best SENSEX = FRAGILE**. **(b)/(c) NOT STARTED and now ON HOLD** — do NOT build the 4-leg live engine or seed a strategy for a FRAGILE structure. The defined-risk condor gives back too much of the naked short-vol edge (wings + year-inconsistency).* Tier 3.
  ...
  - `[ ]` **EDR-10 *(new — the concrete next step, was EDR-02c)*** Seed a paused **NIFTY 3% OTM put spread** strategy (`credit_spread`, `short_otm_pct=0.03`, wing 6, hold-to-expiry / no intraday stop) for forward-paper. Needs a small LIVE-path change: OTM strike selection (`option_selector_v2` sells ATM today) + hold-to-expiry exit (no tp/sl/staleness force-close) on this strategy. 2-leg, reuses the credit_spread execution end-to-end. Then it becomes the first GREEN CANDIDATE_EDGE in the Edge Lab. → forward-paper 3-6 wks → live pilot. Tier 3.
