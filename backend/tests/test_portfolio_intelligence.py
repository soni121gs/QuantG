...
def test_snapshot_is_read_only_and_aggregates_risk_pnl_and_greeks():
    result = build_portfolio_snapshot([
        {"status": "OPEN", "asset_type": "option", "underlying": "NIFTY", "strategy_id": "s1", "max_loss_total": 1000, "updated_at": "2026-01-01T00:00:00Z",
...
def test_equities_are_not_marked_missing_option_greeks():
    result = build_portfolio_snapshot([
        {"status": "OPEN", "asset_type": "equity", "underlying": "RELIANCE", "max_loss_total": 100},
    ], [], now=datetime(2026, 1, 2, tzinfo=timezone.utc))

    assert result["greeks"]["delta"]["total_positions"] == 0
    assert "GREEK_COVERAGE_INCOMPLETE" not in [alert["code"] for alert in result["risk_alerts"]]
