    ...
    <main className="qd-page space-y-5">
      ...
      <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        <SectionPanel title="What-if stress lab" subtitle="Approximate Greek stress; never an order recommendation.">
          ...
          {scenario && <div className="border-t border-[var(--qd-border)] p-4"><div className="flex items-center justify-between"><span className="text-xs text-[var(--qd-text-2)]">Estimated P&L impact</span><span className={`font-mono text-lg font-bold ${Number(scenario.estimated_pnl || 0) >= 0 ? "text-[var(--qd-profit)]" : "text-[var(--qd-loss)]"}`}>{scenario.estimated_pnl === null ? "NOT COMPUTABLE" : money(scenario.estimated_pnl)}</span></div><div className="mt-2 text-xs text-[var(--qd-text-3)]">Computed: {scenario.coverage.computed}/{scenario.coverage.total}; missing inputs: {scenario.coverage.missing_inputs}</div><div className="mt-3 grid gap-3 sm:grid-cols-2">{[["underlying", "By underlying"], ["strategy_id", "By strategy"]].map(([key, title]) => <div key={key}><div className="mb-1 text-xs font-semibold text-[var(--qd-text-2)]">{title}</div>{(scenario.breakdown?.[key] || []).map((row) => <div key={row.name} className="flex justify-between text-xs"><span>{row.name}</span><span className={Number(row.estimated_pnl) >= 0 ? "text-[var(--qd-profit)]" : "text-[var(--qd-loss)]"}>{money(row.estimated_pnl)}</span></div>)}</div>)}</div></div>}
        </SectionPanel>
      </div>
