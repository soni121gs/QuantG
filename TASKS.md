...
- `[ ]` **IA-3** IV surface & skew from settle prices — extend `core/market_context` (RES-2): full put/call SKEW + term-structure from the settle chain we already store (today only IV−RV level is used). Skew-steepening = directional early-warning; term inversion = event/fat-tail flag → feeds the EVENT stand-down + side selection. Pure, historically reconstructable, OOS-testable as a gate.
- `[ ]` **IA-4** Free India-specific data ingest — new importers + stores (mirror bhavcopy pattern, writable `./data`): FII/DII daily net flows (NSE free), GIFT Nifty overnight gap, India VIX term structure, USDINR. All as historically-reconstructable daily features for the classifier. One importer + store + reader each; host-cron freshness like bhavcopy. NO paid feeds.
- `[ ]` **IA-5** GBM regime classifier — `core/regime_gbm.py`: train LightGBM/XGBoost on index_1m + OI/GEX (IA-1) + IV-skew (IA-3) + FII/VIX (IA-4) features → a CALIBRATED confidence score that augments the threshold RAE-1 classifier. **Founder-gated rebuild** (adds lightgbm to the image). Train/validate walk-forward; the score feeds the router ≥0.90 trend gate. Model artifact versioned in `./data`; inference pure + reconstructable. Observe-only vs the threshold classifier until it beats it OOS.
- `[ ]` **IA-6** RD-Agent-style LLM alpha-miner (the self-improving loop) — `core/research_miner.py` + Hermes: a synthesis unit reads `trade_attribution` + `hermes_lessons`, PROPOSES new factor/gate/geometry configs, each AUTO-GRADED by the OOS judge (`eod_options_backtest`/`regime_conditional_oos`), dead ideas blocked by the lessons store (anti-crowding/anti-decay, AlphaAgent's fix). **Observe-only**: writes ranked candidates to `db.research_candidates`; a human/founder promotes. LLM narrates+proposes, code computes, OOS decides — NEVER trades a proposal un-validated (rule #3). `HERMES_MINER_ENABLED` default off.
- `[ ]` **IA-7** Contextual retrieval (RAG) for Hermes — index `wiki/` + `hermes_lessons` + prior OOS runs so the miner/advisor reasons from OUR OWN validated history (AlphaAgent "inherit validated rationales" = the anti-drift fix). Reuses the existing embeddings path. Read-only.
- `[ ]` **IA-8** Telemetry/UI — surface GEX + OI walls, IV skew, FII flow, classifier confidence, and the miner's ranked candidates in Analytics + `/ops/regime-status`. Read-only research surface (mirror the RegimeOOS panel).
> Build order: IA-1 (GEX, pure) → IA-2 (wire+OOS) → IA-3 (IV surface) → IA-4 (free data) → IA-5 (GBM, founder rebuild) → IA-6 (LLM miner, observe-only) → IA-7 (RAG) → IA-8 (UI). EXPLICITLY DEFERRED (low ROI on a solo VPS, honest): deep-hedging RL (institutional; 6× hedging-error cut but not our scale) and LSTM/deep-sequence models (tree models win on our data size). START = IA-1.
