...
- Original dirty `/opt/QuantG` checkout preserved. Inventory/code backup: `/opt/quantg-backups/audit0909-20260909T130644Z`. The audit release used `audit0909-final.override.yml`. After the authorized reminder schedule update, the active backend is `16749b6` and the compose command uses `-p quantg -f /opt/QuantG/docker-compose.yml -f /opt/quantg-releases/auth0909.override.yml`. Use this clean-release override for this deployment; do not pull/reset the old dirty checkout or deploy it over the release. Original env/data/wiki mounts retained.
