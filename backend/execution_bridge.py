...
def normalize_order_row(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "price": row.get("price"),
        "gross_pnl": row.get("gross_pnl") or row.get("gross_realised_pnl") or row.get("realised_pnl") or row.get("pnl"),
        "charges": row.get("charges") if row.get("charges") is not None else row.get("brokerage"),
        "net_pnl": row.get("net_pnl") if row.get("net_pnl") is not None else row.get("realised_pnl") or row.get("pnl"),
        "trade_quality_score": row.get("trade_quality_score"),
        "quality_score": row.get("quality_score"),
        "quality_readiness": row.get("quality_readiness"),
