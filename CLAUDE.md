...
P1 data foundations are partially landed. `scripts/bhavcopy_ingest.py` now accepts stock F&O instruments (`STO`/`STF`) and configurable `--instr-types`; `BhavcopyStore` reads `STF` futures for `underlying_daily()` and `STO` option rows for `option_chain()`. `core/earnings_calendar.py` + `scripts/earnings_calendar_ingest.py` provide a file-backed earnings calendar and `events_for(symbol,start,end)`. `core/india_flows.py` now has participant-wise F&O OI parsing/storage plus `scripts/participant_oi_ingest.py`. `data.store_coverage` watches earnings and participant-OI freshness. Remaining Phase 1 work is mostly data acquisition/backfill: 2024–2026 stock F&O re-ingest, top-30 earnings dates, 2019–2023 old NSE F&O backfill, participant-OI archive backfill, and Upstox Plus approval for deep options-minute history.
