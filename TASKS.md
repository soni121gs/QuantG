...
- [~] **P1-2** (T2) Earnings calendar: code landed 2026-07-19. Added `core/earnings_calendar.py` and `scripts/earnings_calendar_ingest.py` for CSV/public-source backfills and `events_for(symbol, date_range)`. VPS store check 2026-07-19: 0 RELIANCE events for 2024–2026. Remaining: collect/backfill top-30 F&O result-date CSVs and schedule weekly forward capture.
