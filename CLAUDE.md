...
P1 data foundations are partially landed. `scripts/bhavcopy_ingest.py` now accepts stock F&O instruments (`STO`/`STF`) and configurable `--instr-types`; `BhavcopyStore` reads `STF` futures for `underlying_daily()` and `STO` option rows for `option_chain()`. `core/earnings_calendar.py` + `scripts/earnings_calendar_ingest.py` provide a file-backed earnings calendar and `events_for(symbol,start,end)`. `scripts/earnings_calendar_fetch_nse.py` fetches official NSE board-meeting financial-results dates for the default top-30 F&O names; the backend scheduler refreshes a ±45-day forward window weekly on Saturday 05:00 IST. As of 2026-07-19 the VPS runtime earnings store has 309 deduped events across 193 dates from 2024-01-11 through 2026-07-18 (RELIANCE=11, TCS=10). `core/india_flows.py` now has participant-wise F&O OI parsing/storage plus `scripts/participant_oi_ingest.py`. `data.store_coverage` watches earnings and participant-OI freshness. Upstox Plus is founder-confirmed active as of 2026-07-19; QuantG exposes Plus capability and real store coverage through `GET /api/upstox/data-health`, Broker Keys, Market Hub, Hermes Research Lab, and Hermes read-only tool `get_upstox_data_health`. Remaining Phase 1 work is mostly data acquisition/backfill: 2019–2023 old NSE F&O backfill and participant-OI archive backfill. 2024–2026 stock F&O EOD, top-30 earnings dates, and deep options-minute coverage are verified on VPS.

### 20.3 Phase 2 analytics foundation (2026-07-19)
P2 analytics started. `core/iv_surface.py` builds a pure file-store IV surface from bhavcopy option chains: Black-Scholes bisection IV, per-strike/per-expiry points, near-expiry skew, term summary, and trailing ATM-IV richness z-score. ERL rows now carry explicit `trials_count` and a conservative `deflated_sharpe` proxy; a row can no longer remain `CANDIDATE_EDGE` if its deflated score fails after multiple-testing/sample-size adjustment. `scripts/options_1m_ingest_upstox.py` also gained `--sleep-sec` because the overnight Upstox Plus run hit broker 429s when fetching too quickly. These changes are analytics/data only; live trading remains disabled.
