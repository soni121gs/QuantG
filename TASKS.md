...
- `[~]` **EDR-02** *(a) DONE (2026-07-04, `/tmp/condor_sweep.py`): weekly-cadence condor + 18-config OTM×wing sweep on SENSEX+NIFTY — **0 CANDIDATE_EDGE, best SENSEX = FRAGILE**. **(b)/(c) NOT STARTED and now ON HOLD** — do NOT build the 4-leg live engine or seed a strategy for a FRAGILE structure. The defined-risk condor gives back too much of the naked short-vol edge (wings + year-inconsistency).* Tier 3.
  - **→ EDR-09 *(new, the real next question)*** the NAKED short-vol edge is strong (SENSEX +₹2,158/cycle 84%) but undefined-risk; the symmetric condor is fragile. Test cheaper tail-defense that keeps more edge: **one-sided** short spreads (sell only the put side / call side per a slow trend filter), **wider asymmetric wings**, or a **far-tail catastrophe hedge** (buy a cheap deep-OTM wing only, not a full box). Base-rate/OOS style first, no live infra until a config is CANDIDATE_EDGE with all years positive. Tier 3.
