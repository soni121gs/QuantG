...
- **Options Alpha Rebuild pack seeded for PAPER forward-testing (2026-07-05):** `QG-O1`..`QG-O10` are now code templates with `initial_status="live"`, `mode="paper"` enforced at startup, and app-supported structures only (`credit_spread`, `debit_spread`, `single_leg`). This was seeded for founder-directed next-session paper observation, not live approval; promotion still requires OOS plus forward-paper evidence. See [[Options Alpha Rebuild Strategy Pack 2026-07-05]].
...
- **QG-O strategy status:** QG-O1/QG-O3 put-spread theta WORKING (+₹452/+₹816) but fire *unconditionally* in-window (time+bar gate only); QG-O2 trend-gated; **QG-O4 SENSEX Call Spread PAUSED** (no OOS edge on the call side, fights bull drift). **QG-O5..O10 (6 intraday buyers) never fire** — the runner's `_price_history` (server.py ~16994) hardcodes `interval="5minute"` but they were built/OOS-judged for 1-minute; a per-strategy 1-min feed is QUEUED for after-close (opt-in, default stays 5min; results must pass the IMD 1-min OOS gate §14 — no ad-hoc condition loosening).
