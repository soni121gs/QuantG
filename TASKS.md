...
- `[~]` **IA-4** *(FETCHER SHIPPED 2026-07-11 — OOS validation DEFERRED to forward capture)* `core/india_flows.py` + `scripts/india_flows_ingest.py`: NSE `fiidiiTradeReact` FII/DII daily net cash flows → gzip store `data/india_flows` (India-resident VPS, cookie handshake). **Verified live: 2026-07-10 FII +₹2,603cr / DII +₹2,020cr.** Daily forward-capture cron set (18:30, `-u root` like bhavcopy). **BLOCKER for OOS: the public API returns only recent days — no 2024–25 history**, so the base-rate validation (like IA-1/2/3) can't run yet; forward capture accumulates, or a historical FII/DII backfill source is needed. FII net = the highest-conviction INDEPENDENT NIFTY regime signal (directional flow, not the dead OI positioning) — the most promising IA lead, pending data. GIFT Nifty / VIX-term / USDINR: same pattern, add when IA-4 validates. Rule #1 (reconstructable) is why forward capture matters.
