...
## CURRENT STATE & ACTIVE QUEUE (updated 2026-07-14)

**▶▶▶▶▶ ACTIVE PROGRAM (2026-07-14) — QGX: Structured Rebuild (Architecture) — the NEW top priority. Founder-directed after the system teardown (artifact `quantg-redesign-v1`).**
This is an INFRASTRUCTURE program, not a strategy program. It does NOT change what the book trades — it fixes WHY every fix leaks into the next thing. Evidence from a live codebase read (2026-07-14): `server.py`=18,356 lines (80 endpoints, 4 loops, the fill engine); 308 py files / 87k LOC; **84 Mongo collections written, 13 documented**; **408 commits in 30 days (~14/day) = the whack-a-mole treadmill quantified**; 6 writers each on `strategy_positions` + `strategies.today_pnl`; 9 polling loops (2s–300s), no event bus; **8 overlapping research programs (IMD·ERL·HIRB·HSI·RES·EM·RAE·IA), most gated OFF = observe-only debt.** Root cause (one line): the codebase grew FEATURES faster than BOUNDARIES, so every change has a blast radius bigger than the change. The redesign already exists (`CLAUDE.md §11`, written 2026-06-18, "DESIGN ONLY", never executed). QGX executes it, grounded in 2026 practice (modular monolith + vertical slices + Redis Streams — AI agents reason better when logic/data/transactions are co-located and each feature owns its slice).
GOVERNING LAW: (1) **money-correctness before concurrency** — fix source-of-truth + single-writer BEFORE adding the event bus, so the race-bug class is closed first; (2) **every rung ships independently and leaves the app working** — no big-bang; (3) **config-as-data** — book state is a DB row, never a code template or a frozenset; (4) **one writer per state slice** — many readers, one writer (the `trade_fills`/`paper_wallets` template); (5) **ship-or-delete** every gated flag/observe-only module — no new program starts until one is retired; (6) `CORE_ENGINE_LIVE_ENABLED=false` throughout — this is a refactor, not a live-enable. Build 0→7 in order; INFRA-6 (delete observe-only) may run in parallel (it's deletion, low risk).
REUSE MAP: single-writer template already proven on `trade_fills` + `paper_wallets`; `PortfolioLedger.update_position_mark` (ARCH-2A done) is the position-write owner to extend; `position_monitor`/`position_guardian` already refuse to import `server.py` (seams exist); `core/*` pure modules are already cell-shaped. Nothing here needs a new framework — targeted upgrades only.

### ▷ RUNG 0 — Stop the bleeding (discipline + audit, zero-to-low code risk, START HERE)
- `[x]` **INFRA-01** *(DONE 2026-07-14)* Ship-or-delete inventory (real QuantG flags, noise filtered). **KEEP/WIRED:** `CORE_ENGINE_ENABLED/PAPER_ENABLED` (core), `CREDIT_SPREADS_ENABLED`/`DEBIT_SPREADS_ENABLED`, `LOSS_KILLSWITCH_ENABLED`, `PROFIT_LOCK_ENABLED`, `TRAILING_SL_ENABLED`, `EXIT_THETA_AWARE_ENABLED`, `OPTION_DELTA_SELECTION_ENABLED`, `HERMES_TOOL_PLANNER_ENABLED`. **FORWARD-PAPER GATE (founder flip, keep):** `RAE_ROUTER_ENABLED` (default true now), `CORE_ENGINE_LIVE_ENABLED`+`LIVE_SPREADS_ENABLED` (live gate, keep false). **OBSERVE-ONLY / DECISION NEEDED:** `HERMES_ADVICE_ENABLED` (off — Hermes autonomous advice; keep gated), `IV_RANK_GATE_ENABLED`+`IV_RANK_GATE_SHADOW`+`IV_RANK_SHADOW` (shadow gate never graduated → **DELETE candidate INFRA-64**), `ORDERFLOW_GATE_ENABLED`+`ORDERFLOW_GATE_SHADOW`+`ORDERFLOW_SHADOW` (same → **DELETE candidate**), `LEGACY_EXECUTION_WRITES_ENABLED` (legacy fill path → **DELETE candidate**), `CORE_ENGINE_BACKTEST_ENABLED`/`CORE_ENGINE_SHADOW` (verify used). **DELETE-CANDIDATE MODULES (unused):** `core/dealer_positioning.py` (IA verdict DEAD), `core_legacy.py`, `_mongo_position_monitor_loop` (superseded), SQLite `option_state_ledger` (parallel to Mongo). Rule adopted: no new research program until one observe-only item is promoted or deleted. Full execution = INFRA-64.
- `[x]` **INFRA-02** *(DONE 2026-07-14 — guard family CLEAN, no new bugs)* Audited every pre-trade guard. **strategy_runner** `_open_pos`(:836) + `active_position`(:1117): correctly `strategy_id`-scoped ✅. **signal_manager** `SYMBOL_GROUP_ACTIVE_POSITION_EXISTS`(:807) + `SPREAD_POSITION_ALREADY_OPEN`(:910): now strategy+mode-scoped (fixed 77b4f1b/63e8e53) ✅; `active_positions`(:1177) is account-wide but only feeds `one_active_position_per_symbol_group` which DEFAULTS FALSE (multi-strategy safe) ✅. **risk_manager** `open_positions`(:355): account-wide BY DESIGN (portfolio net-delta cap must be) ✅. Runner filter-gates (EXPOSURE_CAP/REGIME_GATE/FREQ_GATE/ENTRY_WINDOW/etc.) all per-strategy via `record_strategy_filter(s["id"],...)` ✅. **VERDICT: the two 07-13 fixes closed the only mis-scoped guards; no further starvation bugs in the guard family.**
- `[x]` **INFRA-03** *(DONE 2026-07-14 — enumerated; full §7 prose deferred to INFRA-51)* ~90 real domain collections written (grep noise from stdlib/pdb/adodbapi excluded). MULTI-WRITER (Rung-2 targets): `strategy_positions` (6: server/runner/monitor/guardian/reconciler/spread_lifecycle → owner = `core/portfolio_ledger`), `strategies.today_pnl` (6 → derive from `trade_fills`). SINGLE-OWNED (safe template): `trade_fills`, `paper_wallets`, `paper_margin_blocks`, `processed_fill_ids`. Domains: exec (strategy_positions/positions/orders/order_events/trades/trade_fills/fills/signals/skipped_signals), wallet/risk (paper_wallets/paper_margin_blocks/risk_state/risk_reservations/*_locks/loss_killswitch_state/profit_lock_state/live_arm_state), market (candles/historical_chains/option_market_ticks/vix_history/market_regime_state/instrument*/broker*), research (daily_reports/trade_attribution/edge_lab_snapshots/edge_math_advice/regime_oos_runs/intraday_options_oos_runs/backtest_runs/research_hypotheses/frozen_evidence), hermes (ai_chats/agent_tool_audit/hermes_*/pending_actions), wiki/user (wiki_docs/users/notifications/system_config). Formal `schema/collections.py` registry + full §7 table = INFRA-51.

### ▷ RUNG 1 — Strategy Registry (config-as-data) — HIGHEST ROI (dissolves 3 of this week's bug classes)
- `[x]` **INFRA-11** *(DONE 2026-07-14)* Registry schema + Pydantic model — `core/strategy_registry.py`: `StrategyConfig`/`VisualConfig`/`OptionsGeometry`/`RiskConfig` (pydantic v2, `extra="ignore"` config projection over the ~90-field runtime doc) + `validate_strategy_doc()` COHERENCE checker that catches the recurring bug classes: the **hold-to-expiry trap** (spread with no tp/sl/time-exit and not marked hold → sits to square-off = the 2026-07-13 RAE regression), tp/sl fraction out-of-range, capital-field mismatch. PURE, no I/O. 10 tests (`tests/test_strategy_registry.py`) + verified against the LIVE book (all 7 strategies validate clean; would have flagged the 3 RAE sellers pre-fix). Maps 1:1 onto strategy docs — migration is a copy.
- `[~]` **INFRA-12** *(FOUNDATION DONE 2026-07-14)* Registry read/validate landed (`project_config`, `validate_strategy_doc`) and WIRED into the config-write path (INFRA-16). REMAINING: the reusable `logic_module` indirection (a registry row → `range_seller`/`trend_delta1`/`credit_scalp` code module) + a startup loader that reads active rows instead of templates — this lands with INFRA-13/14 (the cutover), staged as separate verified commits.
- `[ ]` **INFRA-13** Migrate templates → registry rows. Move `DEFAULT_OPTION_STRATEGIES`, `STANDARD_STRATEGY_CATALOG`, `UPGRADED_DEFAULT_STRATEGY_CODE_BY_NAME`, the per-strategy geometry blocks (`CREDIT_SPREAD_THETA_RISK` etc.) into registry rows. Acceptance: every live/paused strategy exists as a registry row with identical geometry; the code templates are deleted.
- `[ ]` **INFRA-14** Kill the frozenset allowlist. Replace `PAPER_FORWARD_ACTIVE_STRATEGY_NAMES` / `DEAD_STRATEGY_NAMES` / `PAPER_FORWARD_ARCHIVED_STRATEGY_NAMES` (server.py ~1629–1682) + the startup enforcement (~17287–17311) with `registry.active`. Waking/archiving a strategy = flip a boolean in the DB. Acceptance: the frozensets are gone; startup reads `active` from the registry; the 2026-07-13 "half-woken book" class is structurally impossible.
- `[ ]` **INFRA-15** Remove the startup template re-sync (the drift trap). The `_migrate_strategy_code_versions` + template re-sync that silently reverts DB edits goes away once config IS the DB. Acceptance: a DB geometry edit persists across restart with no re-sync step (this is the root of the hold-to-expiry regression + the `visual_config.options.required_capital not re-synced` gotchas).
- `[~]` **INFRA-16** *(WIRED into the Hermes config-write path 2026-07-14)* The Hermes founder-directed config approve path now runs `validate_strategy_doc` on the POST-image and surfaces a `coherence_warning` (routes/ai.py + AIBot.jsx) — so a config edit that would create a hold-to-expiry trap is flagged at write time, not after a losing trade. REMAINING: a dedicated `GET/PUT /api/strategies/{id}/config` admin route + the Strategies-page geometry editor (currently config edits still go via the Hermes lane / DB).
- `[ ]` **INFRA-17** Cutover + tests. Registry-backed book runs a full paper session; catalog tests updated to assert against registry rows not templates. Acceptance: one clean paper day on the registry; old template tests green or replaced.

### ▷ RUNG 2 — Single-writer ownership (ends the phantom-money race class; finishes ARCH-2)
- `[ ]` **INFRA-21** `strategy_positions` single-writer. Route ALL position lifecycle writes (create/transition/close) through `core/portfolio_ledger` (ARCH-2B — mark writes already routed in ARCH-2A). Remove direct `strategy_positions` `update_one`s from `position_reconciler` + the admin routes in `server.py`. Acceptance: exactly one module writes position lifecycle; others read.
- `[ ]` **INFRA-22** `strategies.today_pnl` derived single-source. Make today_pnl a DERIVED read from `trade_fills` (the proven single-owned collection), not a cache 6 modules stamp. Acceptance: today_pnl has one writer (or is computed on read); the 6-writer heat-map row goes to 1.
- `[ ]` **INFRA-23** Remove the reconciler/admin direct writes. Route them through the ledger owner (the canonical `_close_strategy_positions` / ledger API — the 2026-07-06 manual-recovery lesson generalized). Acceptance: no `manual_recovery`-style direct opposite-order or raw status writes remain.
- `[ ]` **INFRA-24** Retire hand-rolled DB locks. `runner_locks` / `strategy_position_locks` / `risk_reservation_locks` exist only to stop writers racing; single-writer deletes the need. Remove the ones now redundant. Acceptance: locks removed where the slice is single-writer; no new races in a paper session.

### ▷ RUNG 3 — Carve server.py into cells (unlocks agent-workability)
- `[ ]` **INFRA-31** Cell scaffolding convention. Define the cell shape: `cells/<name>/handler.py` + `contract.md` (events in/out, state owned) + `tests/`. Document it in `AGENTS.md`. Acceptance: one worked example cell + the convention written down.
- `[ ]` **INFRA-32** Extract the fill/exit engine as the FIRST cell. The highest-value, most-coupled logic (fill handler + exit engine, currently inline in server.py) becomes `cells/execution/`. Acceptance: fill/exit logic lives in a ~500-line cell with its own tests; server.py calls it.
- `[ ]` **INFRA-33** Extract the 80 inline endpoints into routers (batches). Move the inline `@api.*` handlers from server.py into `routes/` by domain (positions, orders, signals, strategies, ops). Do it in reviewable batches. Acceptance: server.py endpoint count trends to ~0; each router is domain-cohesive.
- `[ ]` **INFRA-34** Thin server.py to wiring. What remains is app construction, startup task registration, and dependency injection — no business logic. Acceptance: server.py < ~2,000 lines and contains no fill/exit/endpoint logic.

### ▷ RUNG 4 — Event bus + correlation ids (reaction latency + audit trail)
- `[ ]` **INFRA-41** Stand up Redis + the stream layer. Add Redis to docker-compose (a 6th container); a thin `core/event_bus.py` over Redis Streams (persistent, consumer groups, ack). Acceptance: publish/subscribe works in a smoke test; no trading path touches it yet.
- `[ ]` **INFRA-42** Event schema + correlation/causation ids. Pydantic event models; EVERY event carries `correlation_id` (the trade's whole causal chain) + `causation_id` (the event that caused it). Acceptance: a typed event catalog; ids threaded through the first producer.
- `[ ]` **INFRA-43** Convert the fill loop to events FIRST (prove the pattern on one loop). Replace the ~30s poll→react on fills with a `FillReceived` event the ledger consumes. Acceptance: a fill reacts in <1s; behaviour identical; correlation id visible end-to-end.
- `[ ]` **INFRA-44** Convert the remaining loops. Runner→signal→order→monitor become event-driven using the same contracts. Acceptance: the 9 polling loops become event consumers (keep a slow heartbeat only where genuinely needed).
- `[ ]` **INFRA-45** Event store / audit log. Persist the stream as the append-only event log = deterministic replay for tests + the SEBI audit trail (roadmap Phase 1). Acceptance: any trade's causal chain is queryable by correlation id.

### ▷ RUNG 5 — Schema registry + slice tests (makes agent work CONVERGE)
- `[ ]` **INFRA-51** Formal collection registry + ownership map (promote INFRA-03 to enforced). A `schema/collections.py` registry: each collection → owner cell + Pydantic doc model. Acceptance: a written-but-unregistered collection fails a CI check.
- `[ ]` **INFRA-52** Index + TTL policy. Every collection gets deliberate indexes + retention (signals/audit/heartbeat collections should TTL). Acceptance: no unbounded-growth collection; the startup index-conflict noise is resolved.
- `[ ]` **INFRA-53** Per-cell contract files. Each cell's `contract.md` lists events in/out + state owned = the API across boundaries. Acceptance: every cell has a contract; an agent can load one cell + its contract and work in isolation.
- `[ ]` **INFRA-54** Slice-scoped tests + replay fixtures. Tightly-scoped tests per cell fed by replayed-event fixtures (keeps the LLM honest — the documented fix for agent oscillation). Acceptance: each cell has tests that run without the whole app; CI green.

### ▷ RUNG 6 — Consolidate research into ONE pipeline + delete observe-only debt (clears the background mess; may run in PARALLEL — it's deletion)
- `[ ]` **INFRA-61** Unified evidence pipeline design. Fold IMD·ERL·HIRB·HSI·RES·EM·RAE·IA into ONE flow: `data → judge (OOS/regime-conditional) → forward-paper → promote`, with ONE research ledger + ONE promotion gate. Acceptance: a single documented pipeline; each old program mapped to a stage of it.
- `[ ]` **INFRA-62** Merge the judges. IMD (1-min), ERL (trials), EOD bhavcopy, regime-conditional OOS are all "the judge" — unify their interfaces behind one grader API. Acceptance: one call site grades any strategy on the right judge; duplicate harness code deleted.
- `[ ]` **INFRA-63** Merge sizing+routing. RES (machinery) + EM (sizing) + RAE (regime routing) are one decision engine — collapse the overlapping wiring in `signal_manager._edge_math_spread_size`. Acceptance: one sizing/routing path, not three layered ones.
- `[ ]` **INFRA-64** DELETE observe-only modules (execute the INFRA-01 inventory). Each gated module is WIRED (promoted with a decision) or DELETED. Candidates flagged in code: unused `core/dealer_positioning.py` (IA verdict DEAD), deferred IA-5/6 specs, any `*_SHADOW` gate that never graduated, `core_legacy.py`, superseded `_mongo_position_monitor_loop`, SQLite `option_state_ledger` (parallel to Mongo). Acceptance: `grep observe.only|DESIGN ONLY|nothing imports it` count drops to ~0; TASKS.md shrinks.
- `[ ]` **INFRA-65** One promotion gate. A single `promote_strategy()` path enforcing the ladder (OOS pass → forward-paper → founder gate). No strategy reaches the book except through it. Acceptance: the frozenset/manual seeding paths are gone (depends on INFRA-14).

### ▷ RUNG 7 — Stack upgrades + live hardening (targeted, founder-gated; do on a CLEAN base)
- `[ ]` **INFRA-71** Backtest store → DuckDB + Parquet (from gzip CSV). 10–100× faster OOS sweeps; the judge stops being the bottleneck. Founder-gated rebuild (adds pyarrow/duckdb). Acceptance: existing OOS runs reproduce on the new store, materially faster.
- `[ ]` **INFRA-72** Frontend CRA → Vite. CRA is deprecated (2025). Migrate build tooling (keep React). Acceptance: `docker-compose build frontend` uses Vite; faster HMR/build; app renders identically.
- `[ ]` **INFRA-73** Python tooling → uv + ruff. Fast installs/resolves + one-pass lint/format (keeps agent diffs clean). Acceptance: CI runs ruff; deps managed by uv.
- `[ ]` **INFRA-74** Pydantic v2 contracts everywhere state crosses a boundary (events, registry, API bodies). Acceptance: validation at the boundary, not three hops later; ad-hoc dict passing removed on the hot paths.
- `[ ]` **INFRA-75** OpenTelemetry tracing keyed on correlation id. See a trade's whole chain across cells in one view. Acceptance: a trace spans producer→ledger→exit for one trade.
- `[ ]` **INFRA-76** Live-path hardening + audit trail on the clean base (roadmap Phase 1 founder gate). Only AFTER rungs 0–6. Acceptance: the live pilot checklist (`/ops/rae-live-readiness` + audit log) reads READY on the rebuilt base; `CORE_ENGINE_LIVE_ENABLED` stays a founder flip.
> BUILD ORDER: 0 (discipline, today) → 1 (registry — highest ROI) → 2 (single-writer) → 3 (carve server.py) → 4 (event bus) → 5 (schema+tests) → 6 (consolidate/delete, can parallelize) → 7 (stack+live). Money-correctness (0–2) before concurrency (3–6) before live (7). Full rationale + the 2026-source grounding: artifact `quantg-redesign-v1`; memory `project_qgx_structured_rebuild_07_14`.

**▶▶▶▶ ACTIVE PROGRAM (2026-07-11) — Intelligence Amplification (IA) — sharpen the RAE ensemble with data we already collect + a self-improving research loop. NEW top priority; consumes RAE (§18), does NOT replace it.**
Founder-directed after the strategy-research review. Thesis (honest, no magic switch): the biggest edge is DATA WE ALREADY STORE BUT DON'T USE (per-strike OI/settle in the bhavcopy → dealer positioning/GEX), plus closing the research loop into an RD-Agent-style self-improving miner GATED BY THE EXISTING OOS JUDGE. Every 2026 alpha-mining framework (RD-Agent, AlphaAgent, QuantEvolve) is the loop we already have (hypothesis→backtest→refine); their #1 documented failure is LLMs drifting/overfitting on noisy backtest feedback — our OOS-first law is exactly that guardrail. So these are EDGE-SHARPENING, not a profit switch.
GOVERNING LAW (inherits RAE + OOS-first): (1) every new signal MUST be historically reconstructable (live == backtest) or it can never be OOS-validated — non-negotiable; (2) no signal/factor scales on backtest — OOS → forward-paper → founder-gated live; (3) LLM proposes, CODE computes + the OOS judge decides — a proposal NEVER trades without passing OOS; (4) prefer data we already have (OI/IV surface) over new feeds; add only free/cheap India-specific data (FII/DII, GIFT Nifty, VIX term, USDINR); (5) `CORE_ENGINE_LIVE_ENABLED=false` throughout; observe-only until founder gate.
REUSE MAP: OI/settle already in `core/bhavcopy_store` (494d) + live chain; regime substrate `core/regime_classifier` (RAE-1) + `core/market_context` (RES-2); grader `core/regime_conditional_oos` + `eod_options_backtest`; research memory `trade_attribution` + `hermes_lessons` (HSI); narrator `routes/ai.py` (Hermes). Build 1→8 in order; 1–3 are pure/validatable now, 5–7 need a founder-gated rebuild (lightgbm) + are observe-only.
- `[x]` **IA-1** *(DONE 2026-07-11 — signal built, honest verdict: raw sign NOT a standalone edge on NIFTY)* GEX / dealer-positioning — `core/dealer_positioning.py` (PURE: BS IV-bisection + gamma → net GEX + OI walls + zero-gamma flip + `gex_state`; `from_store` adapter, historically reconstructable). 7 unit tests. **VALIDATION (`scratch/gex_regime_validate.py`, 494 real NIFTY days): the US-convention GEX sign does NOT predict the RAE regime — POSITIVE→RANGE/INSIDE 83.8% (base rate 85%, adds ~nothing), NEGATIVE→TREND/CHOP 11.0% (base 14%, WRONG direction).** Structural reason: US retail is net-LONG options, Indian retail is net-SHORT (sells OTM for income), so the dealer sign is effectively INVERTED/muted on NIFTY. **Do NOT wire raw net GEX sign.** The usable candidates are OI WALLS (pin levels) + magnitude + the inverted convention — as FEATURES for the GBM (IA-5), not a hard gate. Governing law paid off: a plausible signal was validated to near-random before wiring.
- `[x]` **IA-2** *(DONE 2026-07-11 — NEGATIVE verdict: OI/GEX is NOT a usable NIFTY signal)* Follow-up study `scratch/gex_features_study.py` on 494 days, base rate RANGE/INSIDE 85.6%. **(1) inverted sign:** NEGATIVE→range +3.4pts, POSITIVE −1.8pts → NOT usable (<5pt). **(2) OI-wall pin:** 71.8% next-day containment BUT only 46% move toward the walls (random=50%) and band is 4.8% wide → NOT a real pin (a ±2.4% band should contain far more than 71.8% of 1-day moves; directionality is coin-flip). **(3) GEX magnitude→next-day range:** 0.96% vs 0.98% → dead. **CONCLUSION: do NOT wire GEX/OI as a signal, gate, OR GBM feature — it's near-random on NIFTY.** Structural: Indian retail SELLS options, so OI = seller positioning, not directional conviction (US GEX works because retail BUYS). Two rounds of validation killed a hyped signal pre-wire = OOS-first law working. `core/dealer_positioning.py` kept (correct + tested) but UNUSED; revisit only if a paid dealer-flow feed appears. **PIVOT: the OI goldmine is empty for NIFTY → move IA energy to independent data (IA-4 FII/DII is the highest-conviction Indian regime signal; IA-3 IV-skew is directional not positioning).**
- `[x]` **IA-3** *(DONE 2026-07-11 — MARGINAL: skew is a weak feature, not an edge)* IV skew study `scratch/iv_skew_study.py` (494 days). **(a) high put-skew → next-day return:** top-quartile skew 40.3% negative vs 47.3% base (avg +0.17% vs +0.04%) — a weak CONTRARIAN signal (fear-spikes mean-revert slightly up), crosses the 6pt bar. **(b) skew → next-day regime:** NOT usable. **(c) ATM-IV → next-day range:** weak (+0.19pt, below bar). VERDICT: skew is a candidate FEATURE (minor side-selector: don't fade extreme-low skew) but NOT a standalone signal. Not worth wiring alone; hold as a GBM feature IF IA-5 ever runs. Unlike OI it's directional/pricing, so it has *some* signal — just thin.
- `[x]` **IA-4** *(DONE 2026-07-11 — signal DEAD (9yr test), data-infra shipped; see the definitive sub-bullet below)* `core/india_flows.py` + `scripts/india_flows_ingest.py`: NSE `fiidiiTradeReact` FII/DII daily net cash → gzip `data/india_flows` (India-resident VPS, cookie handshake). Live verified 2026-07-10 FII +₹2,603cr/DII +₹2,020cr; daily cron 18:30 (`-u root`). **Backfilled 107 days (2026-01-14..07-10) from the public GitHub MrChartist/fii-dii-data history.json (NSE-sourced, legally clean).** **VALIDATION (`scratch/fii_validate.py`, 102 tradeable pairs vs bhavcopy NIFTY closes to 2026-07): FII net → next-day return corr +0.031 (≈zero, +7.7pt quartile spread < 10pt bar) = NOT a next-day directional signal; DII net weak CONTRARIAN (−15pt spread but corr −0.10, n=26/bucket).** CAVEATS: n=102, 2026-only (single regime), and 1-day direction is the hardest horizon — FII's real reputation is MULTI-DAY / trend-persistence, not next-day. So INCONCLUSIVE-leaning-weak, NOT a clean kill. **To test properly: (a) 2024–25 backfill (Kaggle `manish4348/fii-dii-cash-trading-activity-nse` or `sumansarkar24/nifty-usdinr-fii-dii` — user-download, Kaggle auth-gated) so it aligns with the index_1m regime labels; (b) test REGIME/multi-day horizons, not 1-day direction.** Forward cron keeps accumulating a clean sample regardless. GIFT Nifty/VIX-term/USDINR: add only if FII validates.
  - **REGIME-MAGNITUDE LENS re-checked (2026-07-12, 103 aligned 2026 days, `|FII flow|` → forward realized range, an angle the directional tests didn't isolate):** extreme-flow days precede a 5d |move| of 2.62% vs 1.10% on quiet-flow days (+1.52pt) — the only FII lens not already killed. BUT: (a) directional multi-day on the same 103 days re-derived the EXACT −0.27(10d) contrarian artifact the 9-year test below already refuted, and (b) sign-persistence is dead (below base rate). Given the directional version sign-flips every year out-of-regime, this |flow|→range separation is almost certainly the same single-regime (2026 volatile-cluster) artifact — flow and vol are both high in turbulent months (confound), not a causal lead. **Verdict unchanged: do NOT wire. At most a weak candidate GBM feature IF IA-5 is ever built, and only after a multi-regime |flow|→range test on the 2024–25 backfill. No new edge.**
  - **DEFINITIVE 9-YEAR TEST (2014–2023, 2,133 days: Kaggle FIIDII_cash.csv × Yahoo ^NSEI) — VERDICT: FII/DII is NOT a usable signal.** Multi-horizon corr(FII, fwd return) +0.066(1d)→+0.029(10d) = near-zero & POSITIVE (not contrarian); **per-year corr(fwd-5d) SIGN-FLIPS every year** (2014 −0.09, 2017 +0.09, 2020 +0.21, 2023 −0.16); quartile fwd-5d spread −0.08% (nothing); naive contrarian strat −0.041%/tr, 48.7% win, **−44% over 1066 trades = loses**. **The 103-day 2026 −0.27 "contrarian lead" DID NOT replicate — it was a single-regime artifact (2026 = one more noisy year like 2023).** OOS-first law worked PERFECTLY: a promising thin-sample lead killed by the proper multi-regime test before wiring. CSV archived `data/india_flows/kaggle/` as a possible weak GBM feature only. Fetcher/cron kept (data infra harmless). **FII/DII signal = DEAD as a flow→return predictor. Below is the superseded thin-sample lead.**
  - ~~**DEEP re-test on the same 103 days (`scratch/fii_deep.py`) — the strongest IA lead yet:**~~ *(SUPERSEDED — killed by the 9-year test above)* FII net is a MONOTONIC CONTRARIAN MULTI-DAY signal — corr(FII day d, fwd return) = +0.03 (1d) → −0.10 (2d) → −0.13 (3d) → −0.17 (5d) → **−0.27 (10d)**; 3-day FII-sum → next 3-day return −0.24; same-day +0.15 (flow pushes price, sane); next-day sign-follow 48% (coin → it's a multi-day, not daily, effect). Heavy FII BUYING precedes NIFTY WEAKNESS over 1–2 weeks (opposite of the retail "FII bought → go long"). Economically sensible (buying-climax absorbed by DII/retail → mean-revert). The monotonic-with-horizon shape = the same credibility fingerprint as the RES-2 gate. CAVEATS: n=103, 2026-only, FII net-sellers all window (contrarian could be regime-artifact), overlapping 10d windows inflate significance → SUGGESTIVE not significant. **This is a HYPOTHESIS to confirm on the 2024–25 backfill (test as a multi-day CONTRARIAN filter / side-bias, NOT next-day direction), then forward-paper. Do NOT wire on 103 single-regime days.**
- `[ ]` **IA-5** *(DEFERRED — insufficient validated features to justify a founder-gated lightgbm rebuild)* GBM regime classifier. RATIONALE (evidence-based, 2026-07-11): the IA data-signal sweep found OI DEAD (IA-1/2), IV-skew MARGINAL (IA-3), FII/DII UNVALIDATED (IA-4, no history). Training a GBM now = fitting on near-noise → the over-engineering trap the founder's OOS-first law exists to prevent. **Revisit ONLY after FII/DII (IA-4) forward-validates as a real feature.** `core/regime_gbm.py` + lightgbm rebuild stays spec'd, not built.
- `[ ]` **IA-6** *(DEFERRED — same reason: no validated feature space to mine)* RD-Agent-style LLM alpha-miner. The GRADER (OOS judge, `eod_options_backtest`/`regime_conditional_oos`) already exists and is the reusable core; the deferred part is the LLM proposer. Automating factor proposal over a feature space that has shown mostly noise = automating a search over noise. Revisit when ≥1 new validated feature exists (IA-4). Observe-only `db.research_candidates` design stands.
- `[x]` **IA-7** *(DONE 2026-07-11)* Research RAG — `core/research_rag.py` `reindex_all()` embeds our OWN validated history (wiki_docs + hermes_lessons + regime_oos_runs verdicts + edge_lab snapshots) into the existing `db.hermes_memory` store via `core.embeddings`, so `recall_memory` (and the future IA-6 miner) retrieve what we've PROVEN, not LLM priors (AlphaAgent anti-drift grounding). Idempotent (`rag_key`+`content_hash`), skips zero-vectors, paced `RAG_EMBED_DELAY=1.2s` (avoids Gemini 429). Routes `GET/POST /ops/research-rag[/reindex]`; nightly `scripts/research_rag_reindex.py` (host cron). **Verified: 92 RAG docs indexed (60 wiki/16 lessons/16 OOS); a semantic "is GEX/FII usable on NIFTY" query recalls our own OOS verdicts (QG-O2, NO_EDGE momentum buyer).** Read-only w.r.t. trading.
- `[x]` **IA-8** *(DONE 2026-07-11)* Telemetry/UI — `GET /ops/research-signals` (latest FII/DII + RAG doc count + HONEST tested-dead status per signal). Analytics Edge Lab tab: **RegimeStatus panel** (renders `/ops/regime-status` — current regime per index + confidence, per-strategy ACTIVE/STAND_DOWN + size-mult, P&L by entry regime = the live watch for the RAE ensemble that's now forward-papering) + **ResearchSignals strip** (FII/DII flow + which signals tested DEAD/MARGINAL, so the UI never implies a dead signal is live). Both read-only. Pivoted from the original "surface GEX/skew" spec since those tested dead — surface the live ensemble instead.
> **IA PROGRAM VERDICT (2026-07-11):** the "hidden goldmine in existing data" thesis LARGELY did NOT pan out on NIFTY — options OI is near-random (Indian retail sells options → OI = seller positioning not direction), IV-skew is a thin contrarian feature. The one genuinely promising INDEPENDENT signal is FII/DII flow (IA-4), now capturing forward but not yet OOS-validatable (no history). The heavy ML/LLM infra (IA-5/6/7/8) is correctly DEFERRED — building it on unvalidated near-noise features would be the exact over-engineering the discipline forbids. **This is a SUCCESS of the OOS-first law, not a failure: 4 focused studies, zero noise wired into the live book.** Next real move: accumulate/backfill FII/DII → validate → only then GBM/miner. DEFERRED (low ROI on a solo VPS): deep-hedging RL, LSTM/deep-sequence (tree models win on our data size).
...
- `[x]` **RAE-2** *(DONE 2026-07-10)* REFORM THE JUDGE — `core/regime_conditional_oos.py`: pure `evaluate_regime_conditional(records, target_regime) → RegimeVerdict`. Grades a specialist ONLY on its regime's days, walk-forward WITHIN regime (in-sample = earlier years, OOS = latest), reports OFF-regime give-back separately + a warning when a specialist trades off-regime, counts stand-down days. Thin sample → `NEEDS_FORWARD_PAPER` (never an auto-veto — the fix for the OOS distrust); `NO_EDGE` only when on-regime avg ≤ 0. `RAE_JUDGE_MIN_TRADES=20`. 5 tests. **This is the reformed judge every specialist is graded through.**
- `[x]` **RAE-3a** *(DONE 2026-07-10)* HIGH_VOL_CHOP stand-down — `regime_specialists.chop_stand_down(snapshot)` → True on CHOP/EVENT. **The standout result: on the 65 chop days a naive trader takes 62 trades netting −₹72,066 (avg −₹1,162/day), ~₹114,861 gross losses — standing down AVOIDS all of it.** Cheapest, most certain edge; ship-first pure risk cut. (Router forces size→0 on CHOP at RAE-4.)
- `[x]` **RAE-3b** *(DONE — verdict NEEDS_FORWARD_PAPER)* INSIDE VWAP mean-revert — `regime_specialists.inside_mean_revert` (delta-1, gated to INSIDE/RANGE by RAE-1). **On 125 INSIDE days: +₹103.7/day, but walk-forward IS(2024) −25.5 / OOS(2025) +208.6 → NEEDS_FORWARD_PAPER (honest — one slice negative).** REFINEMENT the judge surfaced: its gate allows INSIDE *or* RANGE, so it also trades RANGE days (off-regime give-back) — tighten ownership to INSIDE-only vs let it co-own RANGE with sellers (design call for RAE-4).
- `[x]` **RAE-3c** *(DONE — verdict NEEDS_FORWARD_PAPER + precision gap identified)* TREND delta-1 long — `regime_specialists.trend_delta1/trend_long/trend_short`, gated by RAE-1 label + **confidence ≥ 0.90** + higher-timeframe (daily SMA20) alignment. **On TREND_UP days: +₹423/day (IS +880 / OOS −33.5), n=4 (thin) → NEEDS_FORWARD_PAPER;** judge WARNS it still leaks −₹458/day on 22 OFF-regime fakeout days. Confirms the study: needs the **IV-cheap gate** (bhavcopy ATM-straddle IV — a follow-up requiring option-data wiring) to tighten precision. NOT vetoed (rare regime) — forward-paper.
- `[x]` **RAE-3d** *(DONE by formalization)* Sellers = the RANGE specialist — `regime_taxonomy.REGIME_OWNER[RANGE]="range_seller"`. The existing option-selling book (QG-O11 etc.) is the RANGE owner; it's option-priced (not delta-1) so it's OOS-validated via the separate EOD/IMD judges, not this index harness. Its live "only trade when RANGE" gating is wired at RAE-4 (router).
  - **RAE-2/3 findings (`scratch/rae_specialists_validate.py`):** the reformed judge works — grades on-regime, walk-forwards, flags off-regime leakage, and returns NEEDS_FORWARD_PAPER (not veto) for thin/mixed samples. Stand-down (3a) is the clearest, cheapest edge. Mean-revert (3b) and trend (3c) are real-but-thin → forward-paper + the two refinements above.
- `[x]` **RAE-4** *(DONE 2026-07-10)* The Router / Capital Allocator — `core/regime_router.py`: pure `route(regime_label, confidence, specialist=) → RoutingDecision(stand_down, size_mult, active_specialists, reasons)`. Enforces regime OWNERSHIP (a `range_seller` stands down — size_mult 0 — when regime ∉ RANGE/INSIDE, i.e. refuses to sell into a TREND/CHOP: the 2026-07-10 loss), CHOP/EVENT hard stand-down, and a TREND confidence gate (≥0.90, size scales with confidence). Stand-down = size_mult 0, not a veto (EM continuous-size philosophy); EdgeMath turns the multiplier into lots. 9 tests. **WIRED observe-only into `signal_manager._edge_math_spread_size`** (`telemetry["router"]`), size applied + entry skipped (`RAE_ROUTER_STAND_DOWN`) ONLY when `RAE_ROUTER_ENABLED=true` (default FALSE — pure annotation, zero trade-path change until the founder flips it). **VALIDATED end-to-end on 498 days (`scratch/rae_router_validate.py`): naive multi-specialist book −₹203,073 → routed (owner-only + chop stand-down) −₹5,583, a +₹197,491 swing, standing down 250/498 days.** The router's edge is knowing when NOT to trade; residual small loss = the delta-1 specialists are still forward-paper (RAE-3), not a router fault. Live confidence feed (fine intraday classifier) lands at RAE-6; live uses the coarse regime for now.
- `[x]` **RAE-5** *(DONE 2026-07-10)* Regime-aware exits — `core/regime_exit.py`: `regime_exit_params(regime)` tunes the RES-3 trailing-lock (TREND → wide trail `giveback 0.70` = let winners run; RANGE → tight `0.40` = book theta fast; INSIDE → tightest `0.35`). Only tunes arm/giveback — hard SL/TP untouched (never less safe). **Wired into `position_monitor._process_spread_position`** (passes regime-tuned arm/giveback to `evaluate_spread_exit`); returns global defaults (byte-identical no-op) unless `RAE_ROUTER_ENABLED=true`. 4 tests.
- `[x]` **RAE-6** *(DONE 2026-07-10)* Ensemble watch view + forward-paper mechanism — `GET /api/ops/regime-status`: current regime per index, per-live-strategy router decision (ACTIVE vs STAND_DOWN + why + size_mult), realized P&L bucketed by entry regime, and the `enforced` flag. Read-only. **Forward-paper is STARTED by the founder setting `RAE_ROUTER_ENABLED=true` in PAPER** (then sellers stand down on TREND/CHOP and the ensemble routes live-in-paper); the endpoint is the watch surface. (Not flipped by an agent — behaviour change to the paper book is a founder call.) Hermes/Analytics can render this endpoint.
- `[~]` **RAE-7** *(GATE BUILT 2026-07-10 — stays founder-gated)* Live pilot readiness — `GET /api/ops/rae-live-readiness`: read-only gate that reports READY/NOT_READY + blocking reasons (forward-paper thin / no positive regime / flags off). **It NEVER enables anything.** Live requires the founder to (a) forward-paper via `RAE_ROUTER_ENABLED=true` until positive regime-conditional evidence (≥`RAE_PILOT_MIN_TRADES=30` closed trades, ≥1 positive regime), then (b) flip `CORE_ENGINE_LIVE_ENABLED=true` + `LIVE_SPREADS_ENABLED=true` + arm the account. `[~]` not `[x]`: the pilot itself only completes when the founder runs it; the gate/checklist is built and will read NOT_READY until then.
