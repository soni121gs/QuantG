...
- `[ ]` **INFRA-01** Freeze new programs + ship-or-delete inventory. Write a table of EVERY gated-off module/flag (`RAE_ROUTER_ENABLED`, EdgeMath advice, `market_context` wiring, IA-5/6 deferred, all `*_SHADOW`/`*_GATE` env) with a column: WIRE-BY date or DELETE. Rule: no new research program until one existing observe-only module is promoted or deleted. Acceptance: the inventory table lives in this file under INFRA-6; each row has an owner decision.
- `[ ]` **INFRA-02** Guard-family audit (the pattern behind this week's 2 fixes). Grep every pre-trade guard in `signal_manager.py`/`strategy_runner.py`/`risk_manager.py` and classify each as user-scoped vs strategy-scoped. Any guard that dedupes/blocks by user+symbol WITHOUT `strategy_id` in a multi-strategy book is a starvation bug (like `SYMBOL_GROUP_ACTIVE_POSITION_EXISTS` + the anti-pyramiding guard, both fixed 2026-07-13). Acceptance: a list of every guard + its scope + verdict (correct / needs strategy_id). Fix the mis-scoped ones.
- `[ ]` **INFRA-03** Collection registry doc (stub). Enumerate all 84 written collections (`grep -rhoE 'db\.[a-z_]+'`), and for each record: purpose, the ONE intended writer, readers, TTL/retention. Land it in `CLAUDE.md §7` (expand the 13-row table to 84). Acceptance: no collection is undocumented; each names a single intended writer (the ones with >1 become INFRA-2 targets).

### ▷ RUNG 1 — Strategy Registry (config-as-data) — HIGHEST ROI (dissolves 3 of this week's bug classes)
- `[ ]` **INFRA-11** Design the registry schema. One `strategy_registry` collection doc = the SOLE source of book state: `{id, name, logic_module, underlying, active(bool), owned_regimes, params{...}, geometry{exit_mode, credit_tp_frac, credit_sl_mult, time_exit_minutes, required_capital, ...}, mode, version}`. Spec the field list + validation (Pydantic model). Acceptance: a written schema + a Pydantic `StrategyConfig` model; maps 1:1 onto the current DB strategy docs so migration is a copy.
- `[ ]` **INFRA-12** Build the registry loader + reusable logic modules. A strategy = a registry row pointing at a reusable `logic_module` (range_seller / trend_delta1 / credit_scalp) — the logic is code, the CONFIG is data. Loader reads registry rows at startup + on change (no redeploy). Acceptance: adding/toggling a strategy is a single DB write; the runner picks it up within one tick.
...
- `[ ]` **INFRA-16** Registry admin API + UI. `GET/PUT /api/strategies/{id}/config` reads/writes the registry with the Pydantic validator; the Strategies page edits geometry directly (replaces DB-poke + redeploy). Fold the Hermes founder-directed config lane (shipped 2026-07-13) onto this same validated write path. Acceptance: founder edits any whitelisted field from the UI; invalid writes rejected at the boundary.
