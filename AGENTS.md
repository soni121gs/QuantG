...
- **▶ ACTIVE PROGRAM (2026-07-08) — Real-Edge System (RES) rebuild:** founder rejected the existing book after a −₹4,571 day (holds ONE position all day, no profit-lock, same bet ×3, QG-O1 sold puts into a `TREND_DOWN`). Building a **dynamic regime-aware seller scalper** (banks profits + trails, re-enters, rotates CE/PE, cuts losers fast) validated on the OOS judge before it trades. 8 tasks, build in order: **RES-1 realistic cost model = DONE** (`06b04c2`, paper spreads now cross the bid/ask — paper P&L reads worse on purpose); **RES-2 = Market Intelligence Engine** (IV−realized_vol edge + regime gate + chain skew + order-flow filter + event calendar, as ONE historically-reconstructable `market_context`). No "foolproof" — target is validated + cost-robust + risk-controlled. Hermes stays researcher, NEVER trader. Full spec: CLAUDE.md §15, queue: TASKS.md RES block.
- **▶▶ ACTIVE PROGRAM (2026-07-09) — EdgeMath (EM): continuous edge-based sizing.** Founder mandate: NO more hard gates/blockers — replace every binary allow/block with a continuous edge→size function. Book sizes UP on real edge + green days, fades toward ZERO as edge decays / day turns red — smoothly. Honest target: `E[daily]>0` + asymmetric bounded loss tail, NOT a guaranteed green day. 3 PURE layers (live+OOS share code): L1 expectancy/Kelly from `trade_attribution`, L2 Kelly-conviction×vol-target lots, L3 day-governor (green-up/red-down/profit-ratchet). **EM-1 DONE** (`core/edge_sizer.py`, 25 tests, pure/isolated). OOS proof (EM-7) BEFORE live wiring (EM-4). Watch `core/profit_lock.py` overlap (EM-5 converts its hard block → size-down). Full spec: CLAUDE.md §16, queue: TASKS.md EM block. Also just landed 2026-07-09: overprotection gates stripped (REGIME_GATE exempt spreads, loss-streak block off, option-quality exempt spreads), midday entry cutoffs removed, hold-to-expiry dropped for QG-O1/O4, time-based spread recycle, RES2 realized-vol data-hole fixed + Jan–Jun 2026 bhavcopy backfilled — see CLAUDE.md §1.
